...
clin_trials = pd.read_csv("Hetionet_prioritization_results/DrugBank-DO-slim-counts.tsv", sep = '\t')
clin_trials["compound_id"] = 'Compound_' + clin_trials["compound_id"]
clin_trials["disease_id"] = 'Disease_' + clin_trials["disease_id"]
clin_trials = clin_trials[clin_trials['disease_id']=="Disease_DOID:1826"]
...
for dir in result_dirs:
    ...
    check = pd.merge(check, clin_trials, left_on='node_id', right_on='compound_id', how='left')
    check = check.iloc[:,  list(range(7)) + [-1]]
    ...
    mxr_not_in_het = pd.merge(mxr_not_in_het, clin_trials, left_on='node_id', right_on='compound_id', how='left')
    mxr_not_in_het = mxr_not_in_het.iloc[:,  list(range(5)) + [-1]]
    ...
    het_not_in_mxr = het_not_in_mxr[["node_id", "name", "Hetionet_score", "Hetionet_rank", "trials"]]
