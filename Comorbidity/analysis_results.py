...
#             plt.annotate(index[i], (Y_sklearn[i,0], Y_sklearn[i,1]))
...
def plot_pca(Y_sklearn, name, percentage, index) :
    ...
    with plt.style.context('seaborn-whitegrid'):
        ...
        for i in range(len(Y_sklearn[:,0])) :
            x = Y_sklearn[i, 0]
            y = Y_sklearn[i, 1]
            text_x = x + 0.2  # Adjust the x-coordinate
            text_y = y + 0.2  # Adjust the y-coordinate
...
def analysis_tissue(heat, n_clus, index, name) :
    ...
    sns.set(font_scale=1)
    Y_sklearn, per = pca(heat)
    plot_pca(Y_sklearn, name + '_heat_pca_tissue', per, index)
...
for h in range(len(multi)) :
    ...
    for k in range(len(tissue)) :
        ...
        #     for j in range(i, disease) :
        #         heat[i,j] = ranked(data[i,k], data[j,k])
        #         heat[j,i] = heat[i,j]
            # print(i)
    ...
    np.save('corrmatrix_' + multi[h], corrmatrix)
    analysis_tissue(corrmatrix, 4, tissue, multi[h])
...
analysis_tissue(final_corrmatrix, 4, tissue, "all_integrated")
